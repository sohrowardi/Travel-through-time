...
function App() {
    ...
    return (
        <main className="bg-black text-neutral-200 min-h-screen w-full flex flex-col items-center justify-center p-4 overflow-hidden relative">
            ...
            <div className="z-10 flex flex-col items-center justify-center w-full h-full flex-1 min-h-0 py-10">
                ...
                {(appState === 'generating' || appState === 'results-shown') && (
                    <div className="relative w-full h-full flex-1 min-h-[600px] flex flex-col" ref={dragAreaRef}>
                        <div className={isMobile ? "flex flex-col gap-12 py-10 items-center overflow-y-auto w-full px-4 scrollbar-hide pb-40" : "absolute inset-0"}>
                            {DECADES.map((decade, index) => {
                                ...
                                return (
                                    <div 
                                        key={decade}
                                        className={isMobile ? "flex justify-center shrink-0" : "absolute"}
                                        style={isMobile ? {} : { 
                                            top: pos.top, 
                                            left: pos.left,
                                            zIndex: 10 + index
                        ...
                        {appState === 'results-shown' && (
                            <motion.div 
                                ...
                                animate={{ opacity: 1, y: 0 }}
                                className="fixed bottom-24 left-1/2 -translate-x-1/2 z-[60] flex flex-wrap justify-center gap-4 w-full px-4"
                            >
                                <button onClick={handleReset} className={secondaryButtonClasses}>
                                    Start Over
                                </button>
                                ...
                                >
                                    {isDownloading ? 'Creating Album...' : 'Download Full Album'}
                                </button>
                            </motion.div>
                        )}
                    </div>
